...
class WebsiteTasks(TaskSet):
    ...
    def getWorkHourList(self,projectId):
        with self.client.get('/workHour/getWorkHoursByProjectID?projectID=%s'%projectId,catch_response=True) as response:
            responseJson=response.json()
            if responseJson['success']:
                response.success()
            else:
    ...
    @task
    def login_applyWorkHour(self):
        self.login()
        projectId=self.selectProject()
        self.getWorkHourList(projectId)
